define(["module", "marionette"], function (module, Marionette) {
  const Context = Marionette.Controller.extend({
    ...
    _getFactory: function (Factory, options, createIfNotFound) {
      ...
      options || (options = {});
      let propertyPrefix = Factory.prototype.propertyPrefix,
      ...
      if (options.internal) {
        ...
        if (objectOptions === undefined && !_.isEmpty(options)) {
          ...
          if (!_.isEmpty(factoryOptions)) {
            options[propertyPrefix] = _.defaults(
              factoryOptions,
              globalOptions[propertyPrefix]
            );
          }
        }
        _.defaults(
      ...
      let propertyName = this._getPropertyName(Factory, nameOptions),
  ...
  function getObject(Factory, options) {
    const factory = this._getFactory(Factory, options, true);
